...
Set *readSet(FILE *read){//ok
  ...
  for(int i = 0;i<new_set->value_max;i++){//preenchendo o vetor
    fscanf(read,"%d",&aux);
...
void writeSet(FILE *write, Set *read){
  ...
  if(read->number_elems != 0){
    for(int z=0;z<=read->value_max/32+1;z++){ //laco pra escrever todos os numeros do conjunto
      for(int y=0;y<32;y++){
        if((read->set[z]&(1<<y)) != 0){
          fprintf(write,"\n%i",((z*32)+y));
        }
...
void copySet(Set *to,Set *copy){
  ...
  to->set = malloc(sizeof(int)*to->value_max/32);
  for(int i=0; i<= to->value_max/32 ; i++){
    to->set[i] = copy->set[i];
  }
}
...
void unionSet(Set *write,Set *read1,Set *read2){
  ...
  if((read1->number_elems != 0)  && (read2->number_elems != 0)){ 
    if(read1->value_max >= read2->value_max){//read1 tem o valor maximo
      ...
      for(aux = 0;aux<=read2->value_max/32; aux++){// adiciona os valores restantes ao write a partir do read1
        write->set[aux] = read1->set[aux] | read2->set[aux];
      }
      for(aux = read2->value_max/32 ;aux<=read1->value_max/32; aux++){// adiciona os valores restantes ao write a partir do read1
      ...
      for(aux = 0;aux<=read1->value_max/32; aux++){// adiciona os valores restantes ao write a partir do read1
        write->set[aux] = read1->set[aux];
      }
      for(aux = read1->value_max/32 ;aux<=read2->value_max/32; aux++){// adiciona os valores restantes ao write a partir do read1
  ...
  if((read1->number_elems != 0)  && (read2->number_elems != 0)){ 
    if(read1->value_max <= read2->value_max){//read1 tem menor valor maximo
      for(aux = read1->value_max; aux >= 0 ; aux--){//descobrindo o valor max que tem em ambos
        if( ((read1->set[aux/32] & (1<<aux%32)) != 0) && ((read2->set[aux/32] & (1<<aux%32)) != 0)){
          write->value_max = aux;
          break;
        }
      }
      write->set = malloc(sizeof(int)*write->value_max/32);
      for(aux = 0; aux <= write->value_max ; aux++){
        if( ((read1->set[aux/32] & (1<<aux%32)) != 0) && ((read2->set[aux/32] & (1<<aux%32)) != 0)){
          write->set[aux/32] = write->set[aux/32] | (1<<aux%32);
          write->number_elems++;
        }
      }
    }else{//read2 tem menor valor maximo
      for(aux = read2->value_max; aux >= 0 ; aux--){//descobrindo o valor max que tem em ambos
        if( ((read2->set[aux/32] & (1<<aux%32)) != 0) && ((read1->set[aux/32] & (1<<aux%32)) != 0)){
          write->value_max = aux;
          break;
        }
      }
